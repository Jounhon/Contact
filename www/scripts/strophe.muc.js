    ...
    Strophe.addConnectionPlugin('muc', {
        ...
        */
        ...
        init: function (conn) {
            ...
            Strophe.addNamespace('X_CONFERENCE', "jabber:x:conference");
        ...
        statusChanged: function (status, condition) {
            if (status === Strophe.Status.CONNECTED || status === Strophe.Status.ATTACHED) {
                this._connection.addHandler(this._onReceiveDirectInviteMessage.bind(this), Strophe.NS.X_CONFERENCE, 'message');
            }
        },
        _onReceiveDirectInviteMessage: function (message) { //收到別人邀請
            console.log('recive invite');
            var x = message.getElementsByTagName('x')[0];
            
            $rootScope.$broadcast('xmpp:muc:invite', {
                jid: x.getAttribute('jid'),
                name: x.getAttribute('name'),
                reason:  x.getAttribute('reason'),
        ...
        */
        ...
        message: function (room, nick, message, html_message, payload, type) {
            ...
            if(payload){
                if(payload.affiliation){
                    msg.c("affiliation", {
                        xmlns: "http://appcomm.com.tw/custom/affiliation",
                        action: payload.affiliation.action,
                        jid: payload.affiliation.jid
                    }).up();
                }
            }
            msg.c("delay", {
        ...
        */
        ...
        groupchat: function (room, message, html_message, payload, type) {
            
            return this.message(room, null, message, html_message, payload,type);
        },
        /*Function
        ...
        */
        ...
        directInvite: function (room, name, receiver, reason, password) {
            console.log('發送邀請');
            var attrs, invitation, msgid;
            msgid = this._connection.getUniqueId();
            attrs = {
                xmlns: Strophe.NS.X_CONFERENCE,
                jid: room
            };
            if (name != null) {
                attrs.name = name;
        ...
        */
        ...
        queryAffiliationMembers: function (room, affiliation, userCallback) {
            var attrs, info;
            attrs = {
            ...
            return this._connection.sendIQ(info,
                                    this._onQueryAffiliationMembersSuccess.bind(this, userCallback),
                                    this._onQueryAffiliationMembersError.bind(this, userCallback));
        },
        _onQueryAffiliationMembersSuccess: function(userCallback, iq)
        {
            var members = [];
            ...
            if (query.length != 0)
            {
                ...
                Strophe.forEachChild(query.item(0), 'item', function (item) {
                    members.push(item.getAttribute("jid"));
                });
            }
            userCallback(members);
        },
        _onQueryAffiliationMembersError: function(userCallback, iq)
        {
            userCallback('Error');
        },
        /*Function
        ...
        */
        ...
        setRoomName: function (room, name, description, success_cb, error_cb) {
            var roomiq;
            roomiq = $iq({
                to: room,
                type: "set"
                ...
                var: "FORM_TYPE"
            }).c("value", null, "http://jabber.org/protocol/muc#roomconfig").up()
            .c("field", {
                var: "muc#roomconfig_roomname"
            }).c("value", null, name).up()
            .c("field", {
                var: "muc#roomconfig_roomdesc"
            }).c("value", null, description);
            console.log(roomiq.toString());
            return this._connection.sendIQ(roomiq.tree(), success_cb, error_cb);
        },
        
        destroyRoom: function (room, success_cb, error_cb) {
            var roomiq;
            roomiq = $iq({
                to: room,
                type: "set"
        ...
        */
        ...
        listRooms: function (server, userCallback) {
            var iq;
            iq = $iq({
                to: server,
                from: this._connection.jid,
            ...
            return this._connection.sendIQ(iq,
                                    this._onListRoomsSuccess.bind(this, userCallback),
                                    this._onListRoomsError.bind(this, userCallback));
        },
        _onListRoomsSuccess: function(userCallback, iq)//接收server傳回來的房間名單
        {
            var rooms = [];
            var query = iq.getElementsByTagName('query');
            if (query.length != 0)
        ...
        _onListRoomsError: function(userCallback, iq)
        {
            userCallback('Error');
        },
